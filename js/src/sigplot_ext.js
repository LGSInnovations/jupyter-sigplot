...
const sigplot = require('sigplot');
const version = require('../package.json').version;


const SigPlotModel = widgets.DOMWidgetModel.extend({
...
const SigPlotView = widgets.DOMWidgetView.extend({
  ...
  handle_command_args_change: function () {
    const old_command_and_arguments = this.model.previous('command_and_arguments');
    const new_command_and_arguments = this.model.get('command_and_arguments');

    // Check that the arrays are different
    if (old_command_and_arguments !== new_command_and_arguments) {
      const command = new_command_and_arguments.command;
      const args = new_command_and_arguments.arguments;
  ...
  handle_done: function () {
    if (this.model.get('done')) {
      const plotLocal = this.plot;
      window.setTimeout(function () {
        const img = plotLocal._Mx.active_canvas.toDataURL("image/png");
